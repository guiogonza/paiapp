...
class ProfitabilityByDriverPage extends StatefulWidget {
  const ProfitabilityByDriverPage({super.key});

  @override
  State<ProfitabilityByDriverPage> createState() => _ProfitabilityByDriverPageState();
}

class _ProfitabilityByDriverPageState extends State<ProfitabilityByDriverPage> {
  ...
  Future<void> _loadDrivers() async {
    final result = await _profileRepository.getDriversWithAssignedVehicle();
    result.fold(
      (failure) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Error al cargar conductores: ${failure.message}')),
          );
        }
      },
      (drivers) {
  ...
  Future<void> _searchRecords() async {
    if (_selectedDriver == null || _fromDate == null || _toDate == null) {
      ScaffoldMessenger.of(context).showSnackBar(
    ...
    try {
      final records = await _profitabilityService.getRecordsByDriver(
  ...
  Future<void> _exportToExcel() async {
    ...
    try {
      final fileName = 'rentabilidad_conductor_${_selectedDriver?.email}_${DateFormat('yyyy-MM-dd').format(_fromDate!)}_${DateFormat('yyyy-MM-dd').format(_toDate!)}.xlsx';
      await ExcelExportUtils.exportProfitabilityRecords(
        records: _records,
        fileName: fileName,
        title: 'Rentabilidad por Conductor - ${_selectedDriver?.email}',
      );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Rentabilidad por Conductor'),
                ...
                DropdownButtonFormField<ProfileEntity>(
                  ...
                  items: _drivers.map((driver) {
                    return DropdownMenuItem(
                      value: driver,
                      child: Text(driver.email ?? 'Sin email'),
                    );
                  }).toList(),
                  onChanged: (value) {
                    setState(() {
